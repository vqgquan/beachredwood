...
eval_control1.py
=================
Loads the trained "rotate" model and runs it with rendering ON, for
...
Usage:
    python eval_control1.py [--model models/best/rotate/best_model.zip]
                             [--episodes 5] [--fps 30]
...
def main():
    ...
    if not os.path.exists(args.model):
        ...
        print(f"[eval_control1] {args.model} not found, falling back to {fallback}")
        args.model = fallback

    print(f"[eval_control1] loading {args.model}")
    ...
    for ep in range(args.episodes):
        ...
        print(f"[eval_control1] episode {ep + 1}/{args.episodes}: "
